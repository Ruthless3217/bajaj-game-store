...
const Speedometer = memo(function Speedometer({ score }) {
    ...
    useEffect(() => {
        ...
        const render = () => {
            ...
            gradient.addColorStop(1, '#10B981'); // Green (High)
            ...
            for (let i = 0; i <= tickCount; i++) {
                ...
                ctx.lineTo(x2, y2);
                ctx.lineWidth = 1.5;
                ctx.strokeStyle = isActive ? 'rgba(255, 255, 255, 0.6)' : 'rgba(255, 255, 255, 0.1)';
                ctx.stroke();
            }
            ctx.restore();
        ...
        <div className="relative flex flex-col items-center justify-center select-none w-full max-w-[280px] mx-auto">
            ...
            <div className="absolute top-[60%] left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none">
                <div className="flex flex-col items-center">
                    <span className="text-[2.5rem] font-black text-white leading-none tracking-tight" style={{ textShadow: '0 0 20px rgba(59, 130, 246, 0.5)' }}>
                        {displayScore}
                    </span>
                    <span className="text-[0.75rem] font-bold text-race-muted uppercase tracking-widest mt-1">
                        Score / 100
                    </span>
                </div>
            </div>
